"""Run backtest with ACTUAL option premiums from DB."""
import os, sys
...
print("\nRunning PREMIUM backtest (real option prices)...")
result = engine.run_with_premiums(df, symbol="NIFTY-I", predictor=predictor)

sep = "=" * 60
print(f"\n{sep}")
print("  PREMIUM BACKTEST - REAL OPTION PRICES")
print(sep)
...
if trades_data:
    ...
    print(tdf[["symbol", "direction", "strategy", "entry_price", "exit_price",
               "stop_loss", "target", "pnl", "result"]].head(10).to_string())

result.export_all(base_name="NIFTY_premium", output_dir="backtest_results")
